...
var CO2_PER_KM_CAR = 0.12;
var CO2_PER_KM_RICKSHAW = 0.03;
...
function calculateCarbonSaved(distance) {
  ...
  return +(carEmissions - rickshawEmissions).toFixed(2);
}
...
async function registerRoutes(app) {
  ...
  app.get("/api/drivers", async (req, res) => {
    ...
    const driversWithLocations = drivers2.map((driver) => ({
      ...driver,
  ...
  app.post("/api/bookings", async (req, res) => {
    ...
    const booking = await storage.createBooking({
      ...
      userId: req.user.id,
      driverId: 1,
      // Assign to first driver for now
      distance,
      carbonSaved
    });
    setTimeout(async () => {
  ...
  app.get("/api/bookings", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    const bookings2 = await storage.getUserBookings(req.user.id);
    res.json(bookings2);
  });
